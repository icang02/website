        ...
        {loading ? (
          ...
          data.map((item, i) => (
            <div className="col-span-12 md:col-span-4" key={i}>
              <a href={"/belajar/" + item.slug} className="group block w-full aspect-video overflow-hidden rounded-xl">
                <div className="w-full h-full bg-center bg-cover group-hover:scale-110 transition-all ease-out duration-500" style={{ backgroundImage: `url(${item.image})` }}></div>
              </a>

              <div className="leading-6">
                <div className="border-l-2 border-r-2 rounded border-gray-50">
                  <div className="px-3">
                    <h6 className="block font-bold mt-5 text-sm text-gray-800">{item.title}</h6>
                    <p className="text-p mt-3 text-xs leading-5">{item.description}</p>
                  </div>
                </div>

                <div className="px-1 flex items-center justify-between mt-5">
                  <div className="border border-blue-500 text-blue-500 rounded px-3 py-1 text-xs font-semibold">2 Bagian</div>

                  <Link href={`belajar/${item.slug}`} className="flex items-center gap-1 text-gray-800 text-sm font-semibold hover:text-blue-500">
