  ...
  useEffect(() => {
    const fetchPartData = async () => {
      try {
        ...
        const resp = await fetch(`${process.env.APP_URL}/api/courses/${courses.id}/${part}`);
  ...
  return (
    <>
      <div className="pt-[88px] px-4 pb-0">
        {loading ? (
          <div className="mt-6 text-sm text-gray-600 text-center">Loading data...</div>
        ) : (
          <>
            ...
            <div className="mt-5">
              ...
              <div className="mt-5">
                <div className="text-sm">{loading2 ? <div className="text-center">Loading content...</div> : parse(coursePart.content)}</div>
