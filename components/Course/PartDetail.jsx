...
export default function PartDetail({ courses }) {
  ...
  return (
    <>
      <div className="mt-3 flex flex-col gap-1">
        {courses.course_part.map((item, i) => (
          <div
            ...
            } cursor-pointer w-full bg-[#F3F4F6] border border-blue-200 rounded px-4 py-2.5 text-sm md:text-base text-p transition-all ease-in-out text-gray-600 hover:bg-blue-400 hover:text-white`}
          >
            {item.title}
          </div>
        ))}
      ...
      <div className="mt-8 text-p">
        <div className="px-0.5 editor">{parse(coursePart.content ?? courses.course_part[0].content)}</div>
